using iTextSharp.text;
using iTextSharp.text.pdf;
using SchoolManagementSystem.Models;
using System.IO;

namespace SchoolManagementSystem.Services
{
    public class PrinciplePdfService
    {
        private readonly IWebHostEnvironment _webHostEnvironment;

        public PrinciplePdfService(IWebHostEnvironment webHostEnvironment)
        {
            _webHostEnvironment = webHostEnvironment;
        }

        public byte[] GeneratePrinciplePdf(List<ApplicationUser> principals)
        {
            using (var memoryStream = new MemoryStream())
            {
                Document document = new Document(PageSize.A4.Rotate(), 40, 40, 80, 60); // Landscape
                PdfWriter writer = PdfWriter.GetInstance(document, memoryStream);
                writer.PageEvent = new PdfHeaderFooter();

                document.Open();

                // Title
                var titleFont = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 16);
                document.Add(new Paragraph("Principle List Report", titleFont)
                {
                    Alignment = Element.ALIGN_CENTER,
                    SpacingAfter = 20
                });

                // Table with 9 columns
                PdfPTable table = new PdfPTable(9)
                {
                    WidthPercentage = 100
                };
                table.SetWidths(new float[] { 1f, 3f, 3f, 4f, 3f, 3f, 3f, 4f, 3f });

                // Table headers
                AddHeaderCell(table, "Sr No");
                AddHeaderCell(table, "First Name");
                AddHeaderCell(table, "Last Name");
                AddHeaderCell(table, "Email");
                AddHeaderCell(table, "Phone Number");
                AddHeaderCell(table, "City");
                AddHeaderCell(table, "Country");
                AddHeaderCell(table, "Address");
                AddHeaderCell(table, "Profile Picture");

                int serialNo = 1;

                foreach (var principal in principals)
                {
                    table.AddCell(serialNo.ToString());
                    table.AddCell(principal.FirstName);
                    table.AddCell(principal.LastName);
                    table.AddCell(principal.Email);
                    table.AddCell(principal.PhoneNumberPublic);
                    table.AddCell(principal.City?.Name ?? "N/A");
                    table.AddCell(principal.Country?.Name ?? "N/A");
                    table.AddCell(principal.Address ?? "");

                    // Profile image column
                    PdfPCell imageCell = new PdfPCell();
                    imageCell.HorizontalAlignment = Element.ALIGN_CENTER;
                    imageCell.VerticalAlignment = Element.ALIGN_MIDDLE;

                    if (!string.IsNullOrEmpty(principal.ProfileImage))
                    {
                        string imagePath = Path.Combine(_webHostEnvironment.WebRootPath, "images", principal.ProfileImage);
                        if (File.Exists(imagePath))
                        {
                            iTextSharp.text.Image profileImage = iTextSharp.text.Image.GetInstance(imagePath);
                            profileImage.ScaleToFit(60f, 60f);
                            profileImage.Alignment = Element.ALIGN_CENTER;
                            imageCell.AddElement(profileImage);
                        }
                    }

                    table.AddCell(imageCell);

                    serialNo++;
                }

                document.Add(table);
                document.Close();

                return memoryStream.ToArray();
            }
        }

        private void AddHeaderCell(PdfPTable table, string text)
        {
            var font = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 10);
            PdfPCell cell = new PdfPCell(new Phrase(text, font))
            {
                BackgroundColor = new BaseColor(211, 211, 211),
                HorizontalAlignment = Element.ALIGN_CENTER,
                Padding = 5
            };
            table.AddCell(cell);
        }
    }
}
